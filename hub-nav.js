 ...
 * Features:
 * • App Switcher pill  — jump between all 5 apps + hub
...
(function () {
  ...
  function detectApp() {
    ...
    if (/ritual/i.test(f)) return 'rituals';
    if (/shop/i.test(f)) return 'shopping';
  ...
  ───────────────────────────────────────── */
  const MANIFEST_VERSION = 11; // bump to rebuild cached search indices
  ...
  const APP_NAV_MANIFEST = {
    ...
    rituals: [
      { label: '🕰️ Today',    navFunc: 'ritualsGo', navArg: 'today',    kind: 'section' },
      { label: '📅 Week',     navFunc: 'ritualsGo', navArg: 'week',     kind: 'section' },
      { label: '🧴 Rituals',  navFunc: 'ritualsGo', navArg: 'routines', kind: 'section' },
      { label: '📈 Insights', navFunc: 'ritualsGo', navArg: 'stats',    kind: 'section' },
    ],
  ...
  // Static entries plus Study's SRS decks, read live so new subjects show up on their own
  function manifestFor(id) {
    const base = APP_NAV_MANIFEST[id] || [];
    if (id !== 'study') return base;
    let decks = [];
    try { decks = Object.keys(JSON.parse(localStorage.getItem('studypro_v7_srs')) || {}); } catch (_) {}
    return base.concat(decks.sort().map((d) => ({ label: '📚 ' + d + ' Deck', navFunc: 'v7OpenDeck', navArg: d })));
  }

  function buildSectionIndex() {
    const appManifest = manifestFor(currentId);
  ...
  function loadProcLibrarySections(procApp) {
    ...
    try {
      ...
      const typeIcon = { anime: '📺', manga: '📚', manhwa: '🖼️', show: '🎬', movie: '🎥', novel: '📖', lightnovel: '📘' };
  ...
  function loadAllIndices() {
    ...
    APPS.forEach((app) => {
      ...
      if (!hasValidIndex) {
        const manifest = manifestFor(app.id);
  ...
  /* ── ROOT ──
     One capsule (like Study's bottom nav pill) instead of two separate
     floating buttons — sits top-right, out of the way of page content,
     rather than bottom-right where it used to crowd shorter pages. */
  #hn-root{
    position:fixed;top:calc(20px + env(safe-area-inset-top,0px));right:20px;z-index:2147483640;
    display:flex;align-items:stretch;
    border-radius:100px;
    border:1px solid rgba(255,255,255,.1);
    background:rgba(8,10,18,.88);
    backdrop-filter:blur(22px);-webkit-backdrop-filter:blur(22px);
    box-shadow:0 4px 28px rgba(0,0,0,.5),0 0 0 1px rgba(255,255,255,.04);
    overflow:hidden;
  }
  #hn-root .hn-divider{width:1px;align-self:stretch;margin:8px 0;background:rgba(255,255,255,.1);flex-shrink:0;}
  ...
  .hn-pill:hover{
    color:#fff;background:rgba(${ACCENT_RGB},.14);
  }
  .hn-pill:active{background:rgba(${ACCENT_RGB},.22);}
  .hn-pill svg,.hn-pill-icon{width:14px;height:14px;flex-shrink:0;opacity:.8;}

  /* ── SWITCHER POPUP ── */
  .hn-sw-wrap{position:relative;display:flex;}
  ...
  const hnDivider = document.createElement('div');
  hnDivider.className = 'hn-divider';
  root.appendChild(hnDivider);
...
return btn;
  ...
  function mountInHubHeader() {
    ...
    (document.getElementById('hub-search-slot') || header).appendChild(searchWrapper);
  }
  ...
  function tryMount() {
    ...
    if (currentId === 'proc') {
      ...
      ensureProcNav();
      setInterval(() => { if (!document.hidden) ensureProcNav(); }, 1000);
      return;
    }
    ...
    if (currentId === 'rituals') {
      // Rituals has its own Switch app / Search buttons in its sidebar and tab
      // bar (they call window.hubNavSwitch / hubNavSearch), so no floating pills.
      root.style.display = 'none';
      document.body.appendChild(root);
      return;
    }
    ...
    if (currentId === 'shopping') {
      // Budget bakes its own Switch App/Search into its header as two
      // more round icon buttons, matching its Sync/Alerts (see Shopping-List.html)
      // instead of this generic pill — so, like Zen/Rituals, it stays off always.
      root.style.display = 'none';
      document.body.appendChild(root);
      return;
    }
